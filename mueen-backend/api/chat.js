...
const allowCors = (res) => {
  ...
  res.setHeader('Access-Control-Allow-Origin', '*'); // للسماح لجميع المصادر، يمكن تقييده لاحقًا
...
export default async function handler(req, res) {
  ...
  // المتصفح يرسل طلب OPTIONS أولاً للتحقق من CORS
  ...
  try {
    ...
    if (!responseData.candidates || !responseData.candidates[0].content) {
      // التعامل مع الأخطاء المحتملة من Gemini مثل حجب المحتوى
